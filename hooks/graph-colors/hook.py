...
def find_script() -> Optional[Path]:
    ...
    script = (
        agentic_kit_root / "skills" / "graph" / "scripts" / "obsidian-graph-colors.py"
    )
    if script.exists():
        return script
...
def main():
    ...
    try:
        ...
        if result.returncode == 0:
            print(json.dumps({"continue": True, "message": "Graph colors updated"}))
        else:
            print(
                json.dumps(
                    {
        ...
        print(
            json.dumps({"continue": True, "message": "Graph colors update timed out"})
        )
    except Exception as e:
        print(
            json.dumps(
                {"continue": True, "message": f"Graph colors error: {str(e)[:100]}"}
            )
        )
